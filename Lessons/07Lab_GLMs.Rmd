...
#Exercise 1:
q1 <- ggplot(EPAair) +
  geom_density(aes(x = Ozone, fill = factor(Year), alpha = .02)) + 
...
q3 <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
...
ozoneAllRegression <- lm(data = EPAAir.subset, PM2.5 ~ . )
summary(ozoneAllRegression)

step(ozoneAllRegression)

lm(formula = PM2.5 ~ mean_Lat + mean_Lng + Month + Ozone, data = EPAAir.subset)
