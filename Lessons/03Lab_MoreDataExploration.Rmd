...
#Always use string as factors = true for reading csv
...
```{r DataExercise-1a}
high_temp <- c(81, 83, 83, 84, 78, 71, 79, 82, 85, 84)
low_temp <- c(55, 56, 58, 60, 61, 63, 63, 64, 64, 63)
```
...
high_temp_C <- to_celcius(high_temp)
low_temp_C <- to_celcius(low_temp)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
Durham_temp <- data.frame(high_temp, low_temp, high_temp_C, low_temp_C)
names(Durham_temp) <- c("High Temperature Fahrenheit","Low Temperature Fahrenheit","High Temperature Celcius", "Low Temperature Celcius")
Durham_temp
```
...
```{r DataExercise-1d}
summary(Durham_temp)
sd(Durham_temp$`High Temperature Fahrenheit`)
sd(Durham_temp$`High Temperature Celcius`)
```
