...
huc8_sf_upper_neuse<- st_read(here('Data/Spatial/NCHUC8.shp')) %>% 
  filter(SUBBASIN == "Upper Neuse")


#Create a map
mapView(huc8_sf, col.regions = "orange") |
mapView(huc8_sf_upper_neuse, col.regions = "purple")
...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% # imports FIPS as factor, strings as factor wouldnt work
  select(COUNTY, FIPS, LOCATION, E_TOTPOP, E_POV, E_MINRTY)

#Check structure
str(svi2018_nc_raw)

#d the SVI attributes to the county spatial features
...
wind_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
  ...
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% #substring, grab from 1st to 8th value (gets HUC8)
  group_by(HUC_8) %>% 
...
neuse_intersect_1 <- counties_sf_utm[neuse_sf,] #same thing as an intersect

#Select intersecting counties using the `filter()` command
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE)) #st_intersects, x = . means whatever gets put in, y = feature
...
myMap + mapview(neuse_counties_sf, alpha.regions = 0) #does not update area, etc
...
the_triangle <- counties_sf_utm %>% 
  filter(NAME %in% c("Catham", "Durham", "Orange", "Wake"))
