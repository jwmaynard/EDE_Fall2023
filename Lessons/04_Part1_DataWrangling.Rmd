...
#StringAsFactors - summary will work with character vars
...
vignette("dplyr")
```
# slice - changes rows based on location
#arrange - changes order of row
#mutate - changes the values of columns and creates new values
### Filter - choose rows based on column loaction
...
```{r arranging data}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) #og dataset, colum to arrange by
...
#mutate(og dataset, new column = calculation)
```
