...
e6 <- ggplot(BR_complete, aes(y = Rainfall, x = Date)) +
  geom_line() +
  geom_smooth()

e6.2 <- ggplot(BR_complete, aes(y = Temperature_C, x = Date)) +
  geom_line() + 
  geom_smooth()
...
rainfall_ts <- ts(BR_complete$Rainfall, start = c(1901-01-01), frequency = 12)
...
temp_ts <- ts(BR_complete$Temperature_C, start = c(1901-01-01), frequency = 12)

rainfall_ts_Decomposed <- stl(rainfall_ts, s.window = "periodic")
#Periodic = default to whatever frequency you specified when time series was created (in our case it was 12)
temp_ts_Decomposed <- stl(temp_ts, s.window = "periodic")

plot(rainfall_ts_Decomposed)
plot(temp_ts_Decomposed)

rainfall_ts_components <- as.data.frame(rainfall_ts_Decomposed$time.series[,1:3])
temp_ts_components <- as.data.frame(temp_ts_Decomposed$time.series[,1:3])
#Grey rectangles show how plot had to zoom in to see the image, bigger grey bar = more zoomed in = smaller impact on dataset
